        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string[] selectedRoles) //, ApplicationUser user)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    m.Body = String.Format("Dear " + user.UserFullName + ", <br/><br/> You have been registered as a user for Hospice Niagara's Employee and Volunteer Portal. This testing phase involves all staff, and we are looking to fill the site with content to prepare for the full launch in the coming weeks! Please make yourself comfortable with the website and get used to navigating it, as well as creating content. <br/><br/> If you haven’t seen it already, please view the annotated video tutorial <a href='https://www.youtube.com/watch?v=2JUwrSgtHxY'>Here</a>. At this point we are treating the site as if it was 100% active, so if you have any announcements, events, or meetings planned, please create them on the portal even if it would be for people that aren’t currently on the portal yet. If you encounter any problems, please fill out a bug report form, which can be found on the portal by searching for it on the resources page, and emailing the filled out form to programmer@hospiceniagara.ca. <br/><br/> <a href=\"{1}\" title= \"User Email Confirmation\">Click this link to confirm your e-mail</a><br /> <br/>To login, use your email address, and your password is set to Last Name/Month of Birth/Date of Birth (e.g. ‘SmithJune2’). You will be prompted to change this on your first log in.<br/><br/>Thank you for your help getting the portal up and running!", user.UserName, Url.Action("ConfirmEmail", "Account", new { Token = user.Id, Email = user.Email }, Request.Url.Scheme));
                    m.IsBodyHtml = true;
